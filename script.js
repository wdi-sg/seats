...
var inputHappened = function (currentInput) {
  if (isNaN(currentInput)) {
    ...
    return "Your ticket is $ " + Math.round((compoundingPrice + Number.EPSILON) * 100) / 100 + " tickets before price change : " +
      befPriceChange;
  } else if (totalPlaneSeats < 6 && totalPlaneSeats > 1) {
    totalPlaneSeats = totalPlaneSeats - 1;
    befPriceChange = totalPlaneSeats - 1;
    compoundingPrice = compoundingPrice * 1.05;
    return "Your ticket is $ " + Math.round((compoundingPrice + Number.EPSILON) * 100) / 100 + " tickets before price change : " + befPriceChange;
  } else if (totalPlaneSeats === 1) {
    totalPlaneSeats = totalPlaneSeats - 1;
    return "Your ticket is $ " + lastSeat + " This is the last ticket available!";
  }
};